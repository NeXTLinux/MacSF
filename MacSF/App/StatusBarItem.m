...
-(void)handler:(id)sender
{
    ...
    switch(((NSMenuItem*)sender).tag)
    {
        ...
        //monitor
        // launch netiquette (with macsf args)
        case monitor:
        {
            //path
            NSURL* path = nil;
            ...
            //launch
            // with args
            if(nil != [NSWorkspace.sharedWorkspace launchApplicationAtURL:path options:0 configuration:[NSDictionary dictionaryWithObject:@[@"-macsf"] forKey:NSWorkspaceLaunchConfigurationArguments] error:&error])
            {
                //err msg
...
-(void)setState
{
    ...
    {
        //update status
        [self.statusItem.menu itemWithTag:status].title = @"MacSF: disabled";
    ...
    {
        //update status
        [self.statusItem.menu itemWithTag:status].title = @"MacSF: enabled";
